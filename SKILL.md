...
- **Codex:** Read [references/codex.md](references/codex.md) and follow that workflow instead of the Claude Code instructions below. Resolve scripts from this skill root. Claude-specific frontmatter, variables, and the task placeholder below apply only to Claude Code.
- **Claude Code:** Follow the original workflow below, including CLAUDE_SKILL_DIR, background Bash calls, and the slash-command arguments.
...
Always pass `--dir` explicitly with the absolute project path. This sets the working directory, not an OS sandbox.
...
- For worker runs, run `git status` and `git diff` in `--dir` and review every change. Remove only the subagent's out-of-scope edits, preserving existing user changes.
